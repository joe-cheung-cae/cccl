...
void test()
{
  cuda::mr::cuda_memory_pool res{};

  { // allocate / deallocate
    void* ptr = res.allocate(42);
  ...
  { // allocate_async / deallocate_async
    ...
    void* ptr = res.allocate_async(42, stream);
    static_assert(cuda::std::is_same<decltype(ptr), void*>::value, "");
  ...
  { // allocate_async / deallocate_async with alignment
    ...
    void* ptr = res.allocate_async(42, 4, stream);
    static_assert(cuda::std::is_same<decltype(ptr), void*>::value, "");
  ...
  { // allocate_async with too small alignment
    while (true)
    {
      ...
      try
      {
        void* ptr = res.allocate_async(5, 42, raw_stream);
        unused(ptr);
      }
  ...
  { // allocate_async with non matching alignment
    while (true)
    {
      ...
      try
      {
        void* ptr = res.allocate_async(5, 1337, raw_stream);
        unused(ptr);
      }
      catch (cuda::std::bad_alloc&)
      {
