...
void test()
{
  ...
  { // comparison against a plain cuda_async_memory_resource
    cuda::mr::cuda_async_memory_resource second{};
    assert(first == second);
    assert(!(first != second));
  }
  ...
  { // comparison against a plain cuda_async_memory_resource with a different pool
    ...
    cuda::mr::cuda_async_memory_resource second{cuda_pool_handle};
    assert(first != second);
    assert(!(first == second));
  }
  ...
  { // comparison against a cuda_async_memory_resource wrapped inside a async_resource_ref<>
    cuda::mr::cuda_async_memory_resource second{};
    assert(first == cuda::mr::async_resource_ref<>{second});
    assert(!(first != cuda::mr::async_resource_ref<>{second}));
    assert(cuda::mr::async_resource_ref<>{second} == first);
    assert(!(cuda::mr::async_resource_ref<>{second} != first));
  }
